-- Add location_id to the return table of get_merchants
-- DROP is required because we are changing the RETURN TABLE signature
...
BEGIN
    ...
    SELECT 
        ...
        l.id AS location_id -- Explicitly return Location ID
    FROM 
    ...
    WHERE 
        ...
        -- STRICT FILTERS RELAXED FOR DEMO (based on previous script)
        -- AND (p_category IS NULL OR o.business_type = p_category)
        
        -- AI Intent Filter: Keywords
        AND (
            p_keywords = '{}' 
            OR EXISTS (
