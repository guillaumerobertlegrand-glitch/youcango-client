-- FIX: Step 5 Validation logic for Restaurants
-- Restaurants do not have services, so checking for 'authorizations' fails.
-- This migration updates Step 5 logic to auto-validate for Restaurants.
...
DECLARE
    ...
    v_ape TEXT;
BEGIN
    -- Common lookup for APE code if needed
    SELECT ape_code INTO v_ape FROM public.organizations WHERE id = p_org_id;
    ...
    CASE p_step
        ...
        WHEN 3 THEN
            ...
            BEGIN
                ...
                -- Check for Restaurant (APE 56...)
                IF v_ape LIKE '56%' THEN
                    -- Restaurant: Needs Price Range ONLY (Duration removed)
        ...
        WHEN 5 THEN
            ...
            -- FIX: Logic depends on business type
            IF v_ape LIKE '56%' THEN
                -- Restaurants: No services -> No skills needed -> Auto Valid
                v_is_valid := TRUE;
                v_details := jsonb_build_object('mode', 'restaurant', 'valid', true);
            ELSE
