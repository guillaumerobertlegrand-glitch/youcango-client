-- Update Validation RPC for Flexible Catalog
    ...
    CASE p_step
        ...
        WHEN 3 THEN
            ...
            DECLARE
                ...
                v_service_count INT;
            BEGIN
                SELECT ape_code, price_range, avg_duration 
                INTO v_ape, v_price_range, v_avg_duration
                FROM public.organizations WHERE id = p_org_id;

                -- Check for Restaurant (APE 56...)
                IF v_ape LIKE '56%' THEN
                    -- Restaurant: Needs Price Range & Duration
                    v_is_valid := (v_price_range IS NOT NULL AND v_avg_duration IS NOT NULL);
                    v_details := jsonb_build_object(
                        'mode', 'restaurant',
                        'has_price_range', (v_price_range IS NOT NULL),
                        'has_duration', (v_avg_duration IS NOT NULL)
                    );
                ELSE
