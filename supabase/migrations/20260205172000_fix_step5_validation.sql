...
BEGIN
    -- Retrieve Business Type (Source of Truth)
    SELECT business_type INTO v_business_type FROM public.organizations WHERE id = p_org_id;
    ...
    CASE p_step
        ...
        WHEN 5 THEN
            -- Step 5: SKILLS (Logic Fixed 2026-02-05)
            -- Rely on Business Type.
            IF v_business_type IN ('merchant', 'restaurant') THEN
                 -- Merchants don't have skills/services matrix. Auto-valid.
                 v_is_valid := TRUE;
                 v_details := jsonb_build_object('mode', 'merchant', 'valid', true);
            ELSE
                -- 'service' or default (fallback)
                DECLARE
                ...
                BEGIN
                    ...
                    v_is_valid := v_auth_exists;
                    v_details := jsonb_build_object('mode', 'service', 'has_skills', v_auth_exists);
                END;
            END IF;
