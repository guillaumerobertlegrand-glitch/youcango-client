-- Add Specialty Label to get_merchants RPC
-- Used to display "Japonais", "Italien" on Map Markers.
...
CREATE OR REPLACE FUNCTION api_v1_get_merchants(
    ...
    p_viewer_id UUID DEFAULT NULL -- Added missing parameter from previous migration signature check
)
RETURNS TABLE (
    id UUID,
    location_id UUID,
...
BEGIN
    ...
    SELECT 
        ...
        o.business_type,
        o.category,
        cs.label AS specialty_label, -- NEW
        l.address,
    ...
    LEFT JOIN
        public.config_specialties cs ON o.specialty_id = cs.id -- NEW JOIN
    WHERE 
