...
export default function ClientPaymentPage() {
    ...
    useEffect(() => {
        ...
        const checkStatus = async () => {
            const { data: session } = await supabase
                .from('sessions')
                .select('amount, payment_status, monetization_model')
                .eq('id', sessionId)
                .single();
    ...
    // Merchant Auto-Redirect Timer (Top-Level Hook)
    // Must be at the top level to adhere to Rules of Hooks
    useEffect(() => {
    ...
    }, [monetizationModel, router]);

    // Merchant Completion Mode (C6M)
    if (monetizationModel === 'subscription') {
        // Auto-redirect handled by top-level useEffect
